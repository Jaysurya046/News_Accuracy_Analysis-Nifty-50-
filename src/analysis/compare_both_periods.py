...
def compare_for_period(pattern, out_prefix):
    ...
    if not files:
        files = sorted([f for f in glob.glob(os.path.join(PROJECT_ROOT, pattern)) if os.path.basename(f) != os.path.basename(NIFTY_FILE)])
    ...
    out1 = os.path.join(PROCESSED_DATA_DIR, f'{out_prefix}_merged_range_comparison.csv')
    ...
    out2 = os.path.join(PROCESSED_DATA_DIR, f'{out_prefix}_best_source_per_day.csv')
    ...
    out3 = os.path.join(PROCESSED_DATA_DIR, f'{out_prefix}_best_source_summary.csv')
