#!/usr/bin/env python3
"""
Generate 1-year Nifty 50 trade setups for all 10 news sources.
Saves with suffix `_1year.csv` to keep separate from 3-year datasets.
~250 trading days per source = ~2,500 total records.
"""
...
try:
    from src.config import SOURCES_1Y_DIR
except ImportError:
    from config import SOURCES_1Y_DIR
...
def generate_nifty_setups_1yr(source_name, num_days=250):
    """Generate 1-year Nifty 50 trade setups."""
...
# Sources
sources = [
    ('Bloomberg', 'bloomberg_nifty50_1year.csv'),
    ('CNBC-TV18', 'cnbc_nifty50_1year.csv'),
    ('ET Now', 'etnow_nifty50_1year.csv'),
    ('Mint', 'mint_nifty50_1year.csv'),
    ('NDTV Profit', 'ndtvprofit_nifty50_1year.csv'),
    ('Times Now Business', 'timesnowbusiness_nifty50_1year.csv'),
    ('India Today Business', 'indiatodaybusiness_nifty50_1year.csv'),
    ('Money Control', 'moneycontrol_nifty50_1year.csv'),
    ('Financial Express', 'financialexpress_nifty50_1year.csv'),
    ('Economic Times Live', 'economictimeslive_nifty50_1year.csv'),
]

print("Generating 1-year Nifty 50 trade setups (no weekends/holidays)...\n")

total_records = 0

for source_name, filename in sources:
    df = generate_nifty_setups_1yr(source_name, num_days=250)
    filepath = os.path.join(SOURCES_1Y_DIR, filename)
    ...
    print(f"✅ {filename}")
    print(f"   Records: {len(df)} trading days (1 year)")
...
print("=" * 70)
print(f"🎯 COMPLETE 1-YEAR PORTFOLIO (10 SOURCES)")
print("=" * 70)
...
print(f"Trading days per source: ~250 (1 year, no weekends/holidays)")
print("=" * 70)
